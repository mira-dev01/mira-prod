    ...
    async with aiohttp.ClientSession() as http_session:
        tts = SarvamHttpTTSService(
            ...
            settings=SarvamHttpTTSService.Settings(
                ...
                language="en-IN",
                # Same phone-call cadence as the live pipeline's own TTS
                # (app/voice/pipeline.py's `pace=1.15` on the real SarvamTTSService
                # construction) -- keeps this asset's delivery consistent with
                # every other spoken line a caller hears from Mira.
                pace=1.15,
            ),
        )
        audio_chunks = []
