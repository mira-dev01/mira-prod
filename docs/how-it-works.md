...
escalate_to_host
  → tools.py closure → tool_handlers.handle_escalate_to_host
      ...
      → asyncio.create_task: _send_escalation_whatsapp  (Twilio WhatsApp, detached)
...
Host sees: dashboard Live Requests (Notification rows) + WhatsApp (if
  Twilio configured) + email (if SMTP unblocked) + Leads Kanban card with escalated=True
...
| 5 | WhatsApp delivery was sandbox-only (24h session window, opt-in required) — no real WhatsApp Business number. | app/integrations/twilio_client.py | **CLOSED (2026-09-12).** Migrated to a real Meta-approved WhatsApp Business number (`whatsapp:+17633436818`), removing the opt-in requirement; `twilio_client.py` also gained Messaging Service SID support. The 24h freeform-message session window itself is a general WhatsApp Business API rule, not sandbox-specific, and still applies — `send_whatsapp`/`send_photos` still have no template fallback for it (tracked separately, not part of this fix). |
