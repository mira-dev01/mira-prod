...
| `search_faq` | Search verified `FaqEntry` rows (property-specific before portfolio-wide); logs a gap (`UnansweredQuestion`) if nothing verified is found. | `handle_search_faq` |
...
- **Voice-specific formatting**: no markdown, one question per response, one response per turn (never simulate the guest's side), concise (1-2 sentences unless reciting a requested list), never repeat the greeting or any prior sentence verbatim, no filler on interruption, dates resolved via a pre-computed "today anchor" (`_today_anchor()`) rather than trusting the LLM's own weekday arithmetic.
- **FAQ-first**: any property/support question must go through `search_faq` — no answering from memory/guesswork; no verified answer → say so and escalate.
