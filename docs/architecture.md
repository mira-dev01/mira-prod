...
│   │   ├── integrations/ Exotel, Twilio (voice + WhatsApp), Clerk, Redis (cache + lease),
│   │   │                 iCal fetcher, Bright Data, SearchApi, Cloudinary, email
...
- **Routers**: every domain router in `app/api/v1/` is mounted under prefix `/api/v1` (see `API_PREFIX` in `main.py`): `auth`, `properties`, `bookings`, `calls`, `guests`, `technicians`, `pricing`, `analytics`, `notifications`, `leads`, `faq`, `negotiation_rules`, `voice`, `take_call`, plus `app/api/v1/webhooks/exotel`.
...
- **Auth**: Clerk owns auth entirely — the previous bcrypt/HS256-JWT/refresh-token system is gone, not just unused (`app/config.py`'s own comment is explicit about this). `app/auth/dependencies.get_current_user` is the FastAPI dependency every protected route uses — verifies the bearer token as a Clerk session JWT (RS256, against Clerk's JWKS endpoint derived from `CLERK_PUBLISHABLE_KEY`, fetched once via a cached `jwt.PyJWKClient`), then resolves/links a local `User` row from the token's `sub` (Clerk user id) via `_resolve_local_user` — matching an already-linked `clerk_user_id`, or backfilling it onto an existing row with the same email (fetched once via the Clerk Backend API, `app/integrations/clerk_client.py`, using `CLERK_SECRET_KEY`) for pre-Clerk accounts, or creating a fresh `User` row on a genuinely new signup. 401s if the token is invalid/expired or the resolved user isn't `status == "active"`. `CLERK_DEV_ORG_ID` names the one Clerk Organization whose members get `is_internal_org=True` (gates internal-only dashboard features like "Talk to Mira"), checked directly against the token's active-org claim (`org_id`, or the newer `o.id`), no extra Clerk API call. `app/api/v1/auth.py` no longer has login/register endpoints — it only has `GET /auth/me`, `POST /auth/onboarding` (fills in fields Clerk's sign-up form doesn't collect) and photo/banner upload, all downstream of a Clerk-authenticated session.

## Frontend

- Next.js 16 App Router, TypeScript, Tailwind CSS, shadcn/ui, `react-day-picker` for calendar UI.
- **API client**: `frontend/src/lib/api.ts`. Single `API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:8000/api/v1"` constant — baked in at build time (Next.js `NEXT_PUBLIC_*` convention), so changing it in production requires a rebuild/redeploy. The `api` object groups typed request functions by domain (`api.auth`, `api.properties`, `api.calls`, `api.guests`, `api.bookings`, `api.pricing`, `api.negotiationRules`, `api.technicians`, `api.notifications`, `api.analytics`, `api.leads`, `api.faq`, `api.faqGaps`, `api.voice`), each calling a shared `request<T>()` helper that attaches `Authorization: Bearer <token>`. The token itself comes from Clerk, not `localStorage`: `setTokenGetter()` (called by `AuthProvider`) hands `api.ts` Clerk's async `useAuth().getToken()`, and the module-level `getToken()` helper calls through it — Clerk's session token is only obtainable from its React hooks, so this is how it reaches every `request`/`uploadFiles`/`uploadAudio` call site without threading it through each one. `request<T>()` throws `ApiError` on non-2xx. `uploadFiles`/`uploadAudio` are separate multipart helpers (no `Content-Type` set manually, so the browser fills in the correct boundary).
- **Auth context**: `frontend/src/lib/auth-context.tsx`. `AuthProvider` wraps the app in Clerk's own provider (`@clerk/nextjs`) and exposes `useAuth()` with `user`, `loading`, `isInternalOrg`, `logout`, `refreshUser` — sign-in/sign-up themselves are Clerk's hosted UI, not local `login`/`register`/`registerHost` calls. On mount, once Clerk reports `isSignedIn`, it calls `api.auth.me()` to hydrate `user`. It also auto-activates a Clerk Organization for a signed-in user with no active org (preferring `NEXT_PUBLIC_CLERK_DEV_ORG_ID` if they're a member, else their first membership), since the backend's `is_internal_org`/org-scoped checks only see an org once it's active in the session. The post-signup Airbnb-import poll (`POST /auth/onboarding`'s Bright Data `snapshot_id`) still stashes a `mira_pending_import` sessionStorage key (`PENDING_IMPORT_KEY`) so the dashboard can resume polling after the redirect — onboarding itself never blocks on that scrape.
...
CLI usage: `railway login` (interactive OAuth, run manually, not from an agent session), then `railway variables`/`railway logs`/`railway deployment list`/`railway up` once linked (`railway link` from `backend/`). Env vars — same set as `config.py` documents (`DATABASE_URL`, `FRONTEND_BASE_URL`, `CLERK_SECRET_KEY`, `CLERK_PUBLISHABLE_KEY`, `CLERK_DEV_ORG_ID`, `LLM_PROVIDER`, `GROQ_*`, `SARVAM_API_KEY`, `EXOTEL_*`, `TWILIO_*`, `SMTP_*`, `SEARCHAPI_API_KEY`, `REDIS_URL`, etc.) — set via `railway variables --set KEY=VALUE` or the dashboard, none checked into `railway.json`. `PORT` is injected automatically.
