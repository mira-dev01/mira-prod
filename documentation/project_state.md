...
- **Account-global host call hours + configurable live-ownership handoff** (`documentation/host-call-hours-and-handoff.md`) — **implemented, uncommitted, not yet run through `pytest`** (the local env can't collect the suite — `import app.main` alone exceeds the runner timeout). Two changes: (1) replaced the temporary hardcoded `FIXED_HOST_HOURS_*` env override's *source of truth* (it still wins as precedence step 1 during rollout) with an editable account-global window on the Settings page — new `User.host_call_hours_enabled/_start/_end/_timezone` columns + migration `a3f1c9d24e08`, `resolve_effective_call_owner(property_, host, now)` gained a `host` param and a host-level branch, stopped reading `Property.call_handling_*` (columns kept in schema, `CallLease` precedent), per-property Call Handling UI removed; (2) `_HOST_HANDOFF_PHRASE` is now host-configurable via `User.agent_handoff_phrase` + `system_prompt.resolve_host_handoff_phrase` (loop-in-host guarded) + a "Live call handoff phrase" field on the AI Training tab, default reworded to "Hold on — the host is available now. I'm passing the call to them." Schema/resolver/prompt logic verified via standalone runs; frontend `tsc --noEmit` passes. **Before merge**: run `pytest` on a working machine; **follow-up**: remove the `FIXED_HOST_HOURS_*` override + `render.yaml` keys once the account-global path is trusted in production.
