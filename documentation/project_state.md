...
- **Response validation without hidden regeneration**: every corrective mechanism (8 guards — `repetition_guard`, `redundant_context_guard`, `meta_commentary_guard`, `escalation_phrase_guard`, `premature_end_call_guard`, `property_recommendation_guard`, `response_shape_guard`, and `end_call_reliability_guard` (added 2026-08-07) — + `StyleComplianceMonitor`) either deterministically rewrites/truncates already-generated text, or nudges the *next* turn's prompt — nothing calls the LLM a second time mid-turn to "fix" a response.
- **Auth: fully migrated to Clerk.** `backend/app/auth/dependencies.py` and `app/integrations/clerk_client.py` — the old local bcrypt/HS256-JWT/refresh-token system is gone entirely, not just unused (see `config.py`'s own comment to that effect). This predates the 2026-08-09 snapshot (Clerk landed `0d052e0`, 2026-07-25) and was already true then; noted here now because the prior snapshot's Status summary never mentioned auth at all.
- **Conversational flexibility / ambiguous-date handling / multi-property recommendations**: implemented — refinement turns ("something cheaper", "closer to Candolim"), relative budget/guest-count resolution, amenity accumulation across turns, match-reason/comparison-note explanations, recommendation diversity rotation. See the 2026-08-05 entries below for the detailed build history. Availability-aware recommendation itself is now the fuller mechanism described immediately below (superseding the earlier, best-effort "availability pre-filtering" phrasing this line used to end on).
- **Availability-first recommendations** (`docs/tasks/availability-first-recommendations.md`, 2026-08-16/17/18) — closes the "recommended-then-rejected" gap where a guest could be recommended a property, react positively, and only then be told it's unavailable via a separate `check_calendar` call. Five pieces: (1) a `nights`-only `ConversationState` slot so a guest with a vague window ("first week of October") is asked for stay length before an exact check-in date, instead of being pressed for one immediately; (2) `recommend_properties`' existing best-effort availability pre-filter made to actually fire consistently (a real, pre-existing `_parse_iso_date` crash bug — triggered whenever `recommend_properties` ran after `check_calendar`/`get_pricing`/`negotiate_rate` had already set a raw `date` object, not a string, into `ConversationState.slots` — was found and fixed along the way); (3) real partial-availability classification (`calendar_service.partial_availability`/`partial_availability_for_candidates`, `full`/`partial`/`none` instead of a hard boolean) so a property with a real-but-partial calendar conflict is surfaced with its actual conflicting dates rather than silently dropped or falsely shown as clean — required adding `window_start`/`window_end` slots alongside `nights`, since an exact check-in/check-out pair can never itself produce a "partial" result; (4) a `LEAD_AGENT_INSTRUCTIONS` sequencing rewrite removing the old separate "have your dates been finalized?" gate now that `recommend_properties` is availability-aware on its own, plus an explicit re-check instruction (`check_calendar` always re-confirms a guest's final exact dates, even off an earlier "full" classification against a looser window); (5) `PropertyRecommendationGuardProcessor` extended with three deterministic (never a second LLM call) checks catching a partial property being falsely claimed available, given a wrong/invented conflicting date, or named with the real dates omitted entirely — the third case is the one real-LLM adversarial testing across this task actually reproduced live (a small fallback model's bare "No." with zero dates named under yes/no pressure; never reproduced on the actual production model, `gpt-oss-120b`, which instead re-verified live). All five implementations, all fifteen tasks (implementation + fresh-context PR review + independent reverify × 5) done, reviewed, and reverified — see that file for the full evidence trail, including two real bugs caught and fixed via direct self-testing during Implementation 5 (a guard-correction bug that silently erased other correct content in the same reply, and a date-matching false-positive/false-negative pair that took three fix attempts to land correctly). Committed on `main` (was uncommitted as of 2026-08-09; confirmed via `git log main -- app/voice/state_prompt_sync.py` and `app/services/calendar_service.py`).
...
- **`CallCoordinator`**: Redis-backed lease/concurrency authority, fully implemented — `acquire_or_reject`/`renew`/`release`/`transfer`, atomic Lua scripts, fail-open-with-loud-logging on Redis outage. Wired into both `run_voice_pipeline` (Exotel) and `run_voice_pipeline_twilio`, including the periodic lease-renewal background task (a previously-shipped-but-uncalled gap, now fixed and called). Committed on `main` (confirmed via `git log main -- app/services/call_coordinator.py`, e.g. `7c701f1`, `1127122`) — was uncommitted as of 2026-08-09.
- **Redis-based call leasing**: complete, not partial. `CallLease` (Postgres) is fully superseded — staged for removal, confirmed nothing writes to it anymore. Committed on `main` — see above.
- **Busy Call Recovery**: fully implemented end-to-end — `RecoveryService`, guest+host WhatsApp (template + plain-text fallback). As of the 2026-08-19 production-WhatsApp-messaging cutover (see below), there is no inbound reply routing anymore — the guest gets a single "call back in 5 minutes" message, not an interactive menu. Recovery Analytics (`GET /analytics/recovery`) still exists; its `recovered`/`avg_recovery_time_seconds` fields are now historical-only. `RecoveryService` itself is committed on `main`; the inbound-reply pieces it used to depend on (`app/services/whatsapp_reply_service.py`, `app/api/v1/webhooks/whatsapp.py`) no longer exist in the tree at all — removed by the 2026-08-19 cutover, not merely uncommitted.
...
- **Dashboard opportunities/recovery UI**: `frontend/src/app/dashboard/opportunities/page.tsx` + `opportunities-card.tsx`/`opportunity-list.tsx`/`recovery-analytics-card.tsx`, real-time via `use-notification-stream.ts`. Committed on `main` (confirmed via `git log main`, e.g. `1fd33e5`, `1127122`) — was uncommitted as of 2026-08-09.
- **Host notifications**: `Notification` model extended with `lead_id` (indexed) + `responded_at`, new channels `busy_recovery`/`busy_recovery_reply` alongside existing `whatsapp`/`escalation`/`system`. Committed on `main` — was uncommitted as of 2026-08-09.
- **Cross-call analytics/learning surfaces** (`docs/tasks/building-intelligence.md`, 2026-08-16) — four pieces closing the gap between "MIRA stores call data" and "MIRA surfaces patterns across calls," each read-only/human-facing, none autonomously feeding back into live pricing/negotiation or the voice pipeline: (1) `CallQualityEvent` — persists `ConversationQuality`'s guard/validator firings per call (previously discarded at call-end) for cross-call querying, written from `on_pipeline_finished` only, `ConversationQuality`'s own live/read-side behavior untouched; (2) `CallSummary.objection_tags` — a controlled 8-value vocabulary (`PRICE_TOO_HIGH`/`DATES_UNAVAILABLE`/etc./`NO_OBJECTION`) extending the existing one-shot post-call summarization prompt, zero new LLM calls; (3) `GET /analytics/quality-events` — guard-firing frequency analytics, modeled directly on `faq_service.faq_gap_analytics`'s delegation pattern; (4) `GET /analytics/objection-insights` + a Pricing-page card — conversion rate by objection tag vs. baseline, with a resolved/unresolved breakdown (a tag fires whether or not the objection was overcome) and a low-sample-size caveat below 5 calls. All four implemented+reviewed; committed on `main` (confirmed via `git log main -- app/models/call_quality_event.py` and `app/api/v1/analytics.py`, e.g. `df33fc2` "building intelligence analytics for agent's components") — was uncommitted as of 2026-08-09. One known gap: the Implementation 4 frontend card's actual rendering was never visually confirmed (see Known limitations below).
- **`PropertyChunk`** (`app/models/property_chunk.py`, committed on `main`) — a new table not previously mentioned in this summary: one row per property per text chunk (overview/amenities/location/house_rules/reviews), each independently embedded (plain JSONB float array, same pattern as `FaqEntry.question_embedding`, not pgvector) so semantic search can be scoped to a chunk type instead of matching one blended per-property vector. Generated at import time from fields the parsers already extract; embeddings backfilled fire-and-forget via `embedding_service.backfill_property_chunk_embedding`, same fail-open stance as other embedding usage in this codebase. Consumed by `app/services/property/retrieval/semantic_search.py`.

### In progress / uncommitted (implemented and tested, not yet merged to `main`)

As of 2026-08-09, the entire Redis-lease/Busy-Call-Recovery/WhatsApp-reply subsystem (`app/models/call_lease.py`, `app/services/call_coordinator.py`, `app/services/recovery_service.py`, `app/utils/webhook_auth.py`, the frontend opportunities pages/components, host-notification changes, and the building-intelligence/analytics surfaces) was real, working code sitting only in a local working tree. **As of 2026-08-25, all of it is confirmed committed on `main`** — re-verified file-by-file via `git log main --oneline -- <path>` for every path named in the 2026-08-09 list; each now shows real commits (see the "Currently implemented" bullets above for the specific commits). The two exceptions are `app/services/whatsapp_reply_service.py` and `app/api/v1/webhooks/whatsapp.py`, which no longer exist anywhere in the tree — the 2026-08-19 production-WhatsApp cutover removed inbound reply routing entirely, so these were never merged as such; they were superseded, not committed. This section is kept (rather than deleted) as a record that the gap flagged on 2026-08-09 has since closed — nothing is currently known to be sitting uncommitted in this repo as of 2026-08-25.

`ConversationStyle`/`ConversationQuality`/`StyleComplianceMonitor`/the streaming response-output rewrite were already committed as of 2026-08-09 (see commits `5a8e1bc`, `1b2b36f`, and later); as of 2026-08-25 the Redis-lease/recovery/opportunities-UI/analytics work above has joined them on `main` too — the whole subsystem list this section used to distinguish "already committed" from "not yet" is now, in effect, all committed.

### Known limitations

- `CallLease` (Postgres table/model/migration) still physically exists in the schema, staged for removal — a future session could mistakenly assume it's active without reading `call_coordinator.py`'s own docstring first. (Still true as of 2026-08-25 — the table has not been dropped.)
- As of 2026-08-19, the code is production-WhatsApp-ready (`TWILIO_WHATSAPP_FROM` no longer has a sandbox default; every proactive send prefers a Content Template, matching what production WhatsApp actually requires outside a 24h session), but no real Meta-approved WhatsApp Business number/account has been provisioned yet — the user does not have WhatsApp Business set up and is buying a Twilio number next. Until that's done and `TWILIO_*` is set to real production values, every WhatsApp send still falls back to the in-app notification only, same as before.
...
- No real/browser voice call has been placed against several of the conversation-architecture phases in this environment (Clerk-only auth + WebRTC not scriptable here) — verification for those relies on direct construction-path runs against real DB data plus `pytest`, not an actual audio call. See `agent-conversation-improvement.md`'s per-phase sign-offs for exactly which. (See also the new, more specific "Talk to Mira" TURN-relay limitation below, discovered while chasing this same class of gap.)
...
- **Partial-availability date-naming is a real, LLM-prompt-following limitation, not fully closed by prompt wording alone**: across multiple real-LLM adversarial samples (`docs/tasks/availability-first-recommendations.md`, Implementations 3-5), the model reliably (100% across every sample) never falsely claims a partial-availability property is fully available — the dangerous direction stays closed — but under direct pressure for a terse yes/no answer, a small/weaker model can give a bare "No." with the real conflicting dates omitted (never reproduced on the actual production model, `gpt-oss-120b`, which instead re-verifies live via `check_calendar`). `PropertyRecommendationGuardProcessor`'s new deterministic append-the-real-dates check (Implementation 5) closes this at the guard layer regardless of which model is talking, so this is fully mitigated in practice — flagged here as a known LLM-reliability characteristic behind the fix, not an open gap in the shipped behavior. The guard's own date-matching regex (`_DATE_RANGE_RE`) has two small, explicitly-accepted, safe-direction (redundant-correction-only, never a missed real violation) gaps: it doesn't recognize an "and"-joined date range ("October 3rd and 5th") or match a wrong-month date sharing the real conflict's day-of-month numbers as genuinely wrong in every input shape probed — see that task's own Task 5.2/5.3 findings for the exact repro cases.
- **"Talk to Mira" browser voice-test is currently broken in the dev environment, unresolved as of 2026-08-25.** Extensive live debugging (2026-08-22 through 2026-08-25, direct STUN/TURN protocol testing with `aioice` plus the Trickle ICE browser tool) confirmed the Metered.ca TURN server (`global.relay.metered.ca` / `standard.relay.metered.ca`, every port/transport combination tried: 80, 443, UDP, TCP) rejects the backend's programmatic TURN Allocate requests with a `400 Bad Request` in every combination tested, while a real browser succeeds with the identical credentials. Ruled out: quota (trial usage was 5KB/500MB), wrong host, wrong port, wrong credentials, and app-code misconfiguration (`app/api/v1/voice.py`'s `_ice_servers()` correctly wires `TURN_URL`/`TURN_URL_TLS`/`TURN_USERNAME`/`TURN_CREDENTIAL`). Root cause still unconfirmed — leading theory is Metered rejecting non-browser/programmatic TURN clients specifically as an anti-abuse measure, unconfirmed. Three fix paths identified but **not yet decided or started**: switch to Twilio's Network Traversal Service (account already has Twilio credentials), self-host coturn, or get Metered support to resolve it. Not fixed as of this writing.
