...
- Twilio WhatsApp moved off the Sandbox to a real WhatsApp Business number (`whatsapp:+17633436818`, 2026-09-12) — the sandbox opt-in requirement no longer applies, but the 24h customer-service-window constraint is a general WhatsApp Business API rule, not sandbox-specific, and still applies to both Busy Call Recovery sends and ordinary `send_whatsapp`/`send_photos`/escalation sends. `twilio_client.py` now also supports sending via a Messaging Service (`TWILIO_MESSAGING_SERVICE_SID`), preferred over a raw `TWILIO_WHATSAPP_FROM` number when both are set.
